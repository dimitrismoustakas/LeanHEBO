...
class LeanHEBO:
    ...
    def _make_surrogate(self) -> ExactGPSurrogate:
        ...
        return ExactGPSurrogate(
            ...
            input_lower=self.space.continuous_lower_bounds.to(self.device),
            input_upper=self.space.continuous_upper_bounds.to(self.device),
        )
    ...
    def _ensure_model(self) -> FitReport | None:
        if (
            ...
            return None
        observations = self.store.materialize()
    ...
    def _fill_unique(
        ...
        while retained < requested and attempts < 8:
            ...
            attempts += 1
        if retained < requested:
            # A finite contextual space can be exhausted. Preserve the requested shape while
            # reporting the condition instead of looping forever.
            missing = requested - retained
            encoded_rows.append(self._draw_sobol(missing, fixed).encoded)
    ...
    def best_y(self) -> float:
        if len(self.store) == 0:
            raise RuntimeError("no data has been observed")
        return float(self.store.y.min().detach().cpu())
